class CarsController < ApplicationController
  ...
  def import
    ...
    if file.nil?
      respond_to do |format|
        format.html { redirect_to new_car_path, :notice => t("rva.cars.controller.import.select") }
        format.json { render :json => t("rva.cars.controller.import.select"), :status => :bad_request, :layout => false }
      end and return
    end

    unless SYS::CSV_TYPES.include?(file.content_type)
      respond_to do |format|
        format.html { redirect_to new_car_path, :notice => t("rva.cars.controller.import.upload") }
        format.json { render :json => t("rva.cars.controller.import.upload"), :status => :bad_request, :layout => false }
      end and return
    end

    if params[:season].nil? || params[:season].empty?
      respond_to do |format|
        format.html { redirect_to new_car_path, :notice => t("rva.cars.controller.import.season") }
    ...
    if params[:category].nil? || params[:category].empty?
      respond_to do |format|
        format.html { redirect_to new_car_path, :notice => t("rva.cars.controller.import.category") }
        format.json { render :json => t("rva.cars.controller.import.category"), :status => :bad_request, :layout => false }
      end and return
    end
