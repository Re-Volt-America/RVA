class CarsController < ApplicationController
  ...
  def superpro
    @cars = Rails.cache.fetch('superpro_cars', :expires_in => 1.month) do
      @cars = cars_of_category(SYS::CATEGORY::SUPER_PRO).sort_by { |car| [car.multiplier, car.stock? ? 0 : 1, car.name] }
    end
  ...
  def clockwork
    @cars = Rails.cache.fetch('clockwork_cars', :expires_in => 1.month) do
      @cars = cars_of_category(SYS::CATEGORY::CLOCKWORK).sort_by { |car| [car.multiplier, car.stock? ? 0 : 1, car.name] }
    end
